...
local function CreateBoss(self)
    ...
    -- Auras
    lum:SetBuffAuras(self, frame, 4, 1, self.cfg.height + 4, 2, "TOPRIGHT",
                     self, "LEFT", -6, self.cfg.height - 3, "BOTTOMRIGHT",
                     "LEFT", "UP", true)
...
if cfg.units[frame].show then
    oUF:RegisterStyle("oUF_Lumen:" .. frame:gsub("^%l", string.upper),
                      CreateBoss)
    ...
    for index = 1, MAX_BOSS_FRAMES or 5 do
        ...
        if index == 1 then
            boss:SetPoint(cfg.units.boss.pos.a1, cfg.units.boss.pos.af,
                          cfg.units.boss.pos.a2, cfg.units.boss.pos.x,
                          cfg.units.boss.pos.y)
        else
            boss:SetPoint("TOP", _G["oUF_LumenBoss" .. index - 1], "BOTTOM", 0,
                          -16)
        end
    end
end
